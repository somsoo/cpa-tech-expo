...
5. 어색한 챗봇 인사말(안녕하세요 등)이나 결언(결론적으로 등), 가짜 경력 사칭은 일체 배제하고, 독자의 실질적인 궁금증과 핵심 신청 요건으로 곧바로 시작하세요.
6. 정부/공공 지원 혜택 및 신청 자격 요건(소득/재산/채무 기준 등)을 구체적인 체크리스트로 명확하게 제시하세요."""
...
7. 초안의 마크다운 비교 분석 표(Table, |---|)와 실전자격 체크리스트를 절대 생략하지 말고 가독성 높은 완성형 표와 불릿 리스트로 반드시 본문에 포함하세요.
